...
export const ContentListItem: React.FC<
  TContentListItem & { index: number; id: string; readOnly?: boolean }
> = ({ id, text, checked, index, readOnly = false }) => {
  const {
    saveContentItem,
    deleteContentItem,
    lastItemId,
  ...
  const handleCheck = useCallback(() => {
    saveContentItem(
      { text, checked: checkboxRef?.current?.checked || false },
      index,
      id
    );
  ...
  const onEnter = useOnEnter(() =>
    addContentItem(defaultContentListItem, index, id)
  );

  return (
    <div className={styles.listItem}>
      {Boolean(text) && !readOnly && (
        <Checkbox
      ...
      {Boolean(text) && !readOnly && (
        <Cross
          ...
          onClick={() => deleteContentItem(index, id)}
        />
      )}
    </div>
  );
